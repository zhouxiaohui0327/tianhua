...
class WeEngine {
    ...
    function set_log($val){
        ...
        file_put_contents(IA_ROOT.'/mylog.txt', $str."\r\n======================================\r\n", FILE_APPEND);
    }
	public function start() {
		global $_W,$_GPC;

        session_write_close();
        $str = preg_replace("/[^?]*\?/i",'',$_SERVER['REQUEST_URI']);
        $strArr = explode('&',$str);
        foreach($strArr as $v){
            $arrTmp = explode('=',$v);
            $_GPC[$arrTmp[0]] = $arrTmp[1];
        }
		/*if(empty($this->token)) {
			exit('Access Denied');
		}*/
		...
		if(strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
            ...
            //消息体签名
            $encryptMsg = $postStr;
            //////$this->set_log($token.'___'.$encodingAesKey.'___'.$appId);
            ...
            //////$this->set_log($msg_sign.'___'.$timeStamp.'___'.$nonce.'___'.$from_xml.'___'.$msg);
            $errCode = $pc -> decryptMsg($msg_sign, $timeStamp, $nonce, $from_xml, $msg);
            if($errCode != 0){
                die($errCode);
            }

			$this->message = WeUtility::parse($msg);
            if($this->message['to'] != 'gh_3c884a361561') {
                //设置信息记录
                set_msg_log($this->message);
                ...
                //设置粉丝最后活跃时间
                set_last_active_time($this->message['from']);
                ...
                ////$this->set_log(str_replace(array_keys($mapping), array_values($mapping), $resp));
                $strResponse = str_replace(array_keys($mapping), array_values($mapping), $resp);
                //$this->set_log($this->message);
                $encryptMsg = '';
                //验证
                if ($this->message['type'] == 'text' && $this->message['content'] == '验证') {
                    $strResponse = '<xml><FromUserName><![CDATA[' . $this->message['to'] . ']]></FromUserName><ToUserName><![CDATA[' . $this->message['from'] . ']]></ToUserName><MsgType><![CDATA[text]]></MsgType><Content><![CDATA[验证通过!]]></Content><CreateTime>' . time() . '</CreateTime><FuncFlag>0</FuncFlag></xml>';
                }
            }
            //全网发布代码start=================================
            if($this->message['to'] == 'gh_3c884a361561'){
                if($this->message['type'] != 'text'){
                    $strResponse = '<xml><FromUserName><![CDATA['.$this->message['to'].']]></FromUserName><ToUserName><![CDATA['.$this->message['from'].']]></ToUserName><MsgType><![CDATA[text]]></MsgType><Content><![CDATA['.$this->message['event'].'from_callback]]></Content><CreateTime>'.time().'</CreateTime><FuncFlag>0</FuncFlag></xml>';
                }
                if($this->message['type'] == 'text'){
                    $strResponse = '<xml><FromUserName><![CDATA['.$this->message['to'].']]></FromUserName><ToUserName><![CDATA['.$this->message['from'].']]></ToUserName><MsgType><![CDATA[text]]></MsgType><Content><![CDATA[TESTCOMPONENT_MSG_TYPE_TEXT_callback]]></Content><CreateTime>'.time().'</CreateTime><FuncFlag>0</FuncFlag></xml>';
                }
                if($this->message['type'] == 'text' && strpos($this->message['content'],'QUERY_AUTH_CODE') !== false){
                    $content = str_replace('QUERY_AUTH_CODE:','',$this->message['content']).'_from_api';
                    $strResponse = '<xml><FromUserName><![CDATA['.$this->message['to'].']]></FromUserName><ToUserName><![CDATA['.$this->message['from'].']]></ToUserName><MsgType><![CDATA[text]]></MsgType><Content><![CDATA['.$content.']]></Content><CreateTime>'.time().'</CreateTime><FuncFlag>0</FuncFlag></xml>';
                }
            }
            //全网发布代码end====================================
...
class WeUtility {
	...
	public static function createModuleProcessor($name) {
		...
		$o = new $classname();
        ////file_put_contents(IA_ROOT.'/mylog3.txt',var_export($o,true)."\r\n",FILE_APPEND);
        return $o;
		/*if($o instanceof WeModuleProcessor) {
            //file_put_contents(IA_ROOT.'/mylog3.txt','module4'."\r\n",FILE_APPEND);
			return $o;
		} else {
            //file_put_contents(IA_ROOT.'/mylog5.txt','module4'."\r\n",FILE_APPEND);
			trigger_error('ModuleProcessor Class Definition Error', E_USER_WARNING);
			return null;
		}*/
        ////file_put_contents(IA_ROOT.'/mylog3.txt','module6'."\r\n",FILE_APPEND);
	}

	public static function createModuleReceiver($name) {
		$classname = "{$name}ModuleReceiver";
        //file_put_contents(IA_ROOT.'/mylog.txt',$classname."\r\n***************\r\n",FILE_APPEND);
		if(!class_exists($classname)) {
            //file_put_contents(IA_ROOT.'/mylog.txt','bu cun zai'."\r\n***************\r\n",FILE_APPEND);
        ...
        //file_put_contents(IA_ROOT.'/mylog.txt',$file."\r\n***************\r\n",FILE_APPEND);
		if(!class_exists($classname)) {
			trigger_error('ModuleReceiver Definition Class Not Found', E_USER_WARNING);
			return null;
		}
        //file_put_contents(IA_ROOT.'/mylog.txt','ffffff'."\r\n***************\r\n",FILE_APPEND);
		$o = new $classname();
        //file_put_contents(IA_ROOT.'/mylog.txt',var_export($o,true)."\r\n***************\r\n",FILE_APPEND);
		if($o) {
			return $o;
		} else {
...
abstract class WeModule {
	...
	protected function createMobileUrl($do, $querystring = array()) {
        ...
        /*$url = createHttpHost(create_url('mobile/module', $querystring));
        return createAppHost($querystring['name'],$url,$querystring['weid']);*/
...
abstract class WeModuleProcessor {
	...
	protected function respNews(array $news) {
		...
		foreach ($news as $row) {
			$response['Articles'][] = array(
                ...
                'Url' => stripos($row['url'],'__auth') === false ? $this->buildSiteUrl($row['url']) : $row['url'],
				'TagName' => 'item',
			);
		}
		return $response;
	...
	protected function buildSiteUrl($url) {
		...
		return createHttpHost(create_url('mobile/auth', $vars));
	}
	...
	protected function createMobileUrl($do, $querystring = array()) {
        ...
        $url = createHttpHost(create_url('mobile/module', $querystring));
        return createAppHost($querystring['name'],$url,$querystring['weid']);
        //return createHttpHost(create_url('mobile/module', $querystring));
		//return create_url('mobile/module', $querystring);
	}
...
abstract class WeModuleSite {
	...
	protected function pay($params = array()) {
		...
		//判断是否可以余额支付
        $cardpay = false;
        if(!empty($_W['account']['payment']['credit']['switch']) && $params['module'] != 'icard' && $_W['fans']['from_user']){
            $cardinfo = pdo_fetch('select id,coin,state from '.tablename('icard_card')." where weid={$_W['weid']} and from_user='{$_W['fans']['from_user']}' limit 1");
            if($cardinfo){
                if($cardinfo['state'] == 0){
                    if($cardinfo['coin'] >= $params['fee']){
                        $cardpay = true;
                    }else{
                        $cardmsg = '您的会员卡余额不足';
                    }
